...
class SpiderManager(object):
    ...
    def init(self):
        ...
        try:
            ...
            rsp = requests.get(request_url, headers=headers, cookies=cookies, proxies=proxies)
            rsp.close()
    ...
    def getList(self, page):
        ...
        try:
            rsp = requests.post(url, headers=headers, cookies=cookies, data=data, proxies=proxies)
        except Exception as e:
            if self.debug:
                print(e)
                print("获取列表页时网络请求出错")
    ...
    def getContent(self, docid):
        ...
        headers['Referer'] = self.url.format(parse.quote(self.conditions))
        try:
            rsp = requests.get(url, headers=headers, cookies=cookies, proxies=proxies)
        except Exception as e:
            if self.debug:
                print(e)
                print("获取内容时网络请求出错")
