...
class SpiderManager(object):
    ...
    def getDocIds(self):
        ...
        list_json = json.loads(self.list)
        # print(list_json)
        ...
        if len(list_json) > 1:
            ...
            for item in list_json[1:]:
                docid = item['文书ID']
                jsa = get_docid.call("GetJs", runeval)
                js_objs = jsa.split(";;")
                jsa1 = js_objs[0] + ';'

                jsa2 = re.findall(r"_\[_\]\[_\]\((.*?)\)\(\);", js_objs[1])[0]
                # jsa1 = jsa1 + jsa2
                jsa2 = jsa2.replace('(!![]+[])', json.dumps(list("true")))
                jsa2 = jsa2.replace('(![]+[])', json.dumps(list("false")))
                jsa2 = jsa2.replace('([![]]+{})', json.dumps(list("false[object Object]")))
                jsa2 = jsa2.replace('(+{}+[]+[]+[]+[]+{})', json.dumps(list("NaN[object Object]")))
                jsa2 = jsa2.replace('[![]]+[][[]]', json.dumps(list("falseundefined")))
                jsa2 = jsa2.replace('[]+[]+{}', json.dumps(list("[object Object]")))
                jsa2 = jsa2.replace('[]+[]+[][[]]', json.dumps(list("undefined")))
                key = get_docid.call("EvalKey", jsa1, jsa2)
                # print(key)
